...
public class Game {
	...
	/**
	 * Constructor necesario para la realización del sprint 4.
	 * @param leftPlayer
	 * @param rightPlayer
	 * @param size
	 * @param debugMod
	 */
	public Game(Player leftPlayer, Player rightPlayer, 
			int size, boolean debugMod) {
		ArgumentChecks.isTrue(leftPlayer != null && rightPlayer != null && size >= 5 && size <= 20,
				"Error: parámetros incorrectos");
		setGameMode(debugMod);
		this.leftPlayer = leftPlayer;
		this.rightPlayer = rightPlayer;
		this.size = size;
		myShip = new Board(size);
	...
	public void play() {
		...
		long time0, timef;
		time0 = System.currentTimeMillis(); //Tomamos la hora en que inicio el algoritmo.
		while (game) {
		 ...
		 timef = System.currentTimeMillis(); //Tomamos la hora de fin del algoritmo.
		 long finalTime = timef - time0;
		 setTime(finalTime);
	...
	/**
	 * Método necesario para saber si hace falta o no repetir turno.
	 * 
	 * @param shoot
	 */
	...
	public void setTime(long finalTime) {
		this.time = finalTime;
	}
